...
def process_hioki_csv(uploaded_file) -> Optional[Tuple[str, pd.DataFrame, pd.DataFrame, pd.DataFrame, List[str]]]:
    ...
    a separate dataframe for removed inactive periods, and a list of warnings.
    """
    try:
    ...
    activity_col = 'L1 Avg Current (A)' if 'L1 Avg Current (A)' in data_df.columns else 'Avg Current (A)'
    ...
    # --- Polarity Issue Detection Block ---
    warnings = []
    power_cols_to_check = [col for col in data_df.columns if 'Real Power' in col and data_df[col].dtype == 'float64']
    pf_cols_to_check = [col for col in data_df.columns if 'Power Factor' in col and data_df[col].dtype == 'float64']

    negative_power_detected = any((data_df[col] < -0.01).any() for col in power_cols_to_check)
    negative_pf_detected = any((data_df[col] < -0.01).any() for col in pf_cols_to_check)

    if negative_power_detected or negative_pf_detected:
        warnings.append(
            "⚠️ **Potential Polarity Issue Detected:**\n\n"
            "Negative values for **real power** and/or **power factor** were found. "
            "This often indicates a reversed current sensor (CT) clamp.\n\n"
            "**Action:** All summary KPIs (e.g., Average Power, Average PF) have been calculated using absolute values. Please review the plotted data and verify the physical measurement setup."
        )
    # --- END Detection Block ---
...
def get_gemini_analysis(summary_metrics, data_stats, params_info, additional_context=""):
    """
    Contacts the Gemini API with a detailed prompt to get an expert analysis.
    """
        ...
        if wiring_system == '1P2W':
            ...
            with tabs[0]:
                ...
                if plot_cols:
                    ...
                    fig_power = px.line(data, x='Datetime', y=plot_cols, title="Power vs. Time")
